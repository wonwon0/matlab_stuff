function [ theta ] = cin_inv_v2( pose_act, theta_act, L)
...
i=1;
    ...
    %----------------------CALCUL THETA1-------------------
    %verfi simgulartie
    if (sqrt((x_d^2+y_d^2))<(L(i,4)*1.1))
        theta_11=theta_act(1);
        theta_12=theta_act(1);
    else
        theta_11 = 2*atan2(-x_d+sqrt(x_d^2+y_d^2-L(i,4)^2),y_d+L(i,4));
        theta_12 = -2*atan2(x_d+sqrt(x_d^2+y_d^2-L(i,4)^2),y_d+L(i,4));
    ...
    t2=z_d-L(i,1);
    t3=(t1^2+t2^2-L(i,2)^2-L(i,3)^2)/(2*L(i,2)*L(i,3));
    if t3^2>0.9999
    ...
    t1=x_d*cos(theta_11)+y_d*sin(theta_11);
    t2=z_d-L(i,1);
    t3=sin(theta_31)*L(i,3);
    t4=L(i,2)+L(i,3)*cos(theta_31);
    %calcul theta_21 avec theta_11 et theta_31
    if sqrt((L(i,2)+L(i,3)*cos(theta_31))^2+(L(i,3)*sin(theta_31))^2)<0.00001
    ...
    t1=x_d*cos(theta_11)+y_d*sin(theta_11);
    t2=z_d-L(i,1);
    t3=sin(theta_32)*L(i,3);
    t4=L(i,2)+L(i,3)*cos(theta_32);
    %calcul theta_22 avec theta_11 et theta_32
    if sqrt((L(i,2)+L(i,3)*cos(theta_32))^2+(L(i,3)*sin(theta_32))^2)<0.00001
    ...
    t1=x_d*cos(theta_12)+y_d*sin(theta_12);
    t2=z_d-L(i,1);
    t3=sin(theta_33)*L(i,3);
    t4=L(i,2)+L(i,3)*cos(theta_33);
    %calcul theta_23 avec theta_12 et theta_33
    if sqrt((L(i,2)+L(i,3)*cos(theta_33))^2+(L(i,3)*sin(theta_33))^2)<0.00001
    ...
    t1=x_d*cos(theta_12)+y_d*sin(theta_12);
    t2=z_d-L(i,1);
    t3=sin(theta_34)*L(i,3);
    t4=L(i,2)+L(i,3)*cos(theta_34);
    %calcul theta_24 avec theta_12 et theta_34
    if sqrt((L(i,2)+L(i,3)*cos(theta_34))^2+(L(i,3)*sin(theta_34))^2)<0.00001
    ...
    for k=1:4
        if (theta(k,1) > pi)||(theta(k,1)<-pi)
            theta(k,1)=theta(k,1)+2*pi;
        elseif (theta(k,2) > pi)||(theta(k,2)<-pi)
            theta(k,2)=theta(k,2)+2*pi;
        end
    end
    %sélection des solution avec le coude vers le haut
    iter=1;
    for k=1:4
        if theta(k,3)<0 && theta(k,3)>-pi
            for j=1:3
                theta_coude_up(iter,j)=theta(k,j);
            end
            iter=iter+1;
        end 
    end
