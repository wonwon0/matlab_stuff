function [ theta ] = cin_inv( pose_act, theta_act, L)
...
for i=1:4
    if (theta(i,1) > pi)||(theta(i,1)<-pi)
        theta(i,1)=theta(i,1)+2*pi;
    elseif (theta(i,2) > pi)||(theta(i,2)<-pi)
        theta(i,2)=theta(i,2)+2*pi;
    elseif (theta(i,3) > pi)||(theta(i,3)<-pi)
        theta(i,3)=theta(i,3)+2*pi;
    end
end
%sélection des solution avec le coude vers le haut
iter=1;
theta_coude_up=[0 0 0;0 0 0];
for i=1:4
    if theta(i,3)<0 && theta(i,3)>-pi
        for j=1:3
            theta_coude_up(iter,j)=theta(i,j);
        end
        iter=iter+1;
    end 
end
