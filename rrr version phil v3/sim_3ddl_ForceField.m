...
disp('auto mode? (Y=oui, A=non)')
while true
    [pos, but] = mat_joy(1);
    if but(4)==1
        auto=1;
        disp('mode automatique')
        pause(0.5)
        break
    elseif but(1)==1
        auto=0;
...
if auto==1
    load('vit_record.mat');
    load('pose_start.mat');
    loaded_vit=vit_record;
    pose_init=pose_start;
end

slide=1;

%Définition des paramètres DH du robot (derniere ligne DOIT être les
%paramètres DH du robot complet (jusqu'à l'effecteur).)
...
set(fig, 'Position', [1920 (1080-720) 1280 720])
...
%%
k=0;
limit=StructureLimites_testbench_force();
...
%      a=[a(1) a(3) -a(2)];
    ...
    if auto==0%controle manuel avec la mannette
        [pos, but] = mat_joy(1);
        a = pos;
    ...
    else%on utilise les données de vitesses loadées au début du script
        but=zeros(1,7);
        if auto>=size(loaded_vit,1)
            if force==1
                save('theta_saved_force.mat','theta_saved')
            else
                save('theta_saved.mat','theta_saved')
            end
            break
        end
        dir=loaded_vit(auto,:);
        auto=auto+1;
        theta_saved=[theta_saved;theta_act];
    end
     
    
    if but(1)==1 %on quite le programe
        run=0;
        if isempty(vit_record)
        else %si on a enregistrer des choses, on les sauvegarde
            save('vit_record.mat','vit_record')
            save('pose_start.mat','pose_start')
        end
    end
    ...
    if but(3)==1 && record==0;
        record=1;
        pose_start=pose_act(1,:);
        disp('record on')
        pause(0.5)
    elseif but(3)==1 && record==1;
        record=0;
        disp('record off')
        pause(0.5)
    end
    if but(4)==1 && force==0
        force=1;
        disp('Mode switched to Force')
        pause(0.5)
    elseif but(4)==1 && force==1
        force=0;
        disp('Mode switched to Vector')
        pause(0.5)
    end
    if but(5)==1
        vit=vit*1.05;
    ...
    for i=1:size(pose_act,1)
        [d_min_t,pose_prox_t,pose_prox_pt_act]=verifDistance_v4(limit,pose_act(i,:),L(i,:),L(1,:),theta_act(1,:));
        d_min=[d_min d_min_t];
        pose_prox=[pose_prox;pose_prox_pt_act];
        poses_prox_pt_act.pose(i).poses=pose_prox_pt_act;
    end
    %on garde en mémoire l'input de l'utilisateur
    ...
    if force==0
        ...
        v_input=verifVitesse_ForceField(v_prem,normale_effecteur,d_min_relevant);
    end
    if slide==0
    ...
    % vérification dépassement de la limitte
%     for i=1:size(pose_act,1)
