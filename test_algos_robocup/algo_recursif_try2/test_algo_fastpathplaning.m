...
pose_obstacle_x = rand(11,1)*5000 + 600;
pose_obstacle_y = pose_obstacle_x +(rand(11,1)-0.5)*2 * 2000;
...
for i = 1:size(pose_obstacle,1)
    d(i) = circle2D(pose_obstacle(i,:),300,[0 0 1], 3);
    hold on
end
...
%     end
% end
vit_obstacle_r = (rand(11,1)-0.5)*2 * 20;
count = 0;
while run
    count = count+0.05;
...
%     end
    ...
    for i=1:length(d)
        ...
        d(i).XData = d(i).XData + vit_obstacle_r(i) * cos(count);
        d(i).YData = d(i).YData + vit_obstacle_r(i) * sin(count);
    end
    tic
    path = FastPathPlaning(pose_obstacle, path, depth, NaN, 1 );
    toc
