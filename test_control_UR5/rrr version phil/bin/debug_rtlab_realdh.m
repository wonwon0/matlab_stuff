...
if state==0
    close all
    addpath('X:\matlab\rrr version phil\geom3d\geom3d')
    addpath('X:\matlab\rrr version phil\geom3d\meshes3d')
    addpath('X:\matlab\rrr version phil\p_poly_dist_v1')
    addpath('X:\matlab\rrr version phil\joystick soft 2')
    addpath('X:\matlab\rrr version phil\Remesher')
...
elseif state==1
    ...
    P3=cin_dir_realdh(theta_init, [0.2755 0.290 0 0.0098]);
    P4=cin_dir_realdh(theta_init, [0.2755 0.290 0.2 0.0098]);
    P5=cin_dir_realdh(theta_init, L);
    %%
    ...
    Q1=[cos(theta_init(1)) -sin(theta_init(1))*cos(pi/2) sin(pi/2)*sin(theta_init(1)); 
        sin(theta_init(1)) cos(pi/2)*cos(theta_init(1)) -sin(pi/2)*cos(theta_init(1)); 
        0 sin(pi/2) cos(pi/2)];
    Q2=[-1 0 0; 
        0 cos(theta_init(2)) sin(theta_init(2)); 
        0 sin(theta_init(2)) -cos(theta_init(2))];
    Q3=[1 0 0; 
    ...
    v2=[cos(0) 0 sin(0); 0 1 0; -sin(0) 0 -cos(0)]*v2';
    ...
    v3=[cos(0) -sin(0) 0; sin(0) cos(0) 0; 0 0 1]*v3;
    ...
    v4=v4*[cos(pi) -sin(pi) 0; sin(pi) cos(pi) 0; 0 0 1];
    v4_1=(Q3*v4');
    v4_1=(Q2*v4_1);
    v4_1=(Q1*v4_1)';
    ...
    theta_next=cin_inv_realdh(pose_next_act,theta_act, L);
    theta_next=theta_act;
%     jacob=jacobian_loc(theta_act, L);
%     vit_ang=inv(jacob)*v_input'*5;
%     theta_next=theta_act+(vit_ang')*dt;
    ...
    P3=cin_dir_realdh(theta_next_save, [0.2755 0.290 0 0.0098]);
    P4=cin_dir_realdh(theta_next_save, [0.2755 0.290 0.2 0.0098]);
    P5=cin_dir_realdh(theta_next_save, L);
    
    %rotation de la diférence de distance avec la derniere itération
    
    Q1=[cos(theta_next_save(1)) -sin(theta_next_save(1))*cos(pi/2) sin(pi/2)*sin(theta_next_save(1)); 
        sin(theta_next_save(1)) cos(pi/2)*cos(theta_next_save(1)) -sin(pi/2)*cos(theta_next_save(1)); 
        0 sin(pi/2) cos(pi/2)];
    Q2=[1 0 0; 
        0 cos(theta_next_save(2)) sin(theta_next_save(2)); 
        0 sin(theta_next_save(2)) -cos(theta_next_save(2))];
    Q3=[1 0 0; 
    ...
    delta_theta = signedDelta_theta(theta_next_save,theta_act);
