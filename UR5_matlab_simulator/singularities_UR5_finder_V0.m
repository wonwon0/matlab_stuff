...
if abs(theta_act(3)) < tol
    normales_effecteur = [normales_effecteur gram_schmidth(jacob_eff(:,[1,2,4,5,6]))];
end

if (abs(theta_act(5)) < tol) || (abs(theta_act(5)) > (pi - tol))
    normales_effecteur = [normales_effecteur gram_schmidth(jacob_eff(:,[1,2,3,4,6]))];
end
